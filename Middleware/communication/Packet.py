...
class Packet:
    ...
    def turn_on_logging():
        Packet.IS_LOGGING_ON = True
    
    def turn_off_logging():
        Packet.IS_LOGGING_ON = False
    ...
    def unpack(self, bytes_ : bytes):
        ...
        for v, u in zip(self.__values, struct.unpack(pattern, bytes_)):
            ...
            if(Packet.IS_LOGGING_ON):
                Packet.LOGS[v.name].append(u)
    
    def process_packet(id : c_uint16, bytes_ : bytes):
        if id in ALL_PACKETS:
            Packet.IS_RECEIVING = True
    ...
    def plot(name : str):
        x = [i for i,_ in enumerate(Packet.LOGS[name])]
        plt.plot(x, Packet.LOGS[name][0:len(x)])
        plt.title(name)
        plt.show()
